...
def parse_text(duck_text, lang_file, bunny):
    ...
    for line in duck_text.split('\n'):
        if len(line) > 0:
            ...
            if DEBUG:
                print("CMD:%s Instruction: %s" %(cmd,instruction))
            ...
            for i in range(repeat_count):
                if cmd == 'STRING':
                    for char in instruction:
                        ...
                        if DEBUG:
                            debug_output = char, ': ', convert_hex(elements[2]), convert_hex(elements[0])
                            print(debug_output)
                    ...
                    if DEBUG:
                        debug_output = instruction, ': ', convert_hex(elements[2]), convert_hex(elements[0])
                        print(debug_output)
                else:
